...
@section('content')
    <div class="admin-content__header">
        <h2 class="admin-content__title">Добавление баннера</h2>
    </div>
    <div class="admin__form">
        <form method="post" action="{{ route('panel.banners.store') }}" class="" enctype="multipart/form-data">
            @csrf
            ...
            <div class="admin-content__section">
                ...
                <div class="admin__form-input-block">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" id="title" class="input admin__form-input" required />
                    @error('title')
                ...
                <div class="admin__form-input-block">
                    ...
                    <textarea class="input admin__form-input" name="min_descr" id="min_descr" rows="5" required>{{ old('min_descr') }}</textarea>
                    @error('min_descr')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="admin__form-input-block">
                    ...
                    <input type="text" name="link" id="link" class="input admin__form-input">
                    @error('link')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
